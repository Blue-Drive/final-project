...
int main(int argc, char** argv) {
	...
	//african_head.obj
	Model model1("C:\\Users\\Christian\\source\\repos\\Tiny Raster\\resources\\african_head.obj"); //turns out a cstring is a char pointer 
...
void ZTriangle(Vec3f* pts, int* zBuffer, TGAImage& image, const TGAColor& color) {
	...
	Vec3f Pf;
	for (Pf.x = bboxMin.x; Pf.x <= bboxMax.x; Pf.x + (2.9 / image.get_width())) {
		for (Pf.y = bboxMin.y; Pf.y <= bboxMax.y; Pf.y + (2.0 / image.get_height())) {
			...
			if (zBuffer[int(P.x + P.y * image.get_width())] < Pf.z) {
				zBuffer[int(P.x + P.y * image.get_width())] = Pf.z;
	...
	for (P.x = bboxMin.x; P.x <= bboxMax.x;P.x++) {
		for (P.y = bboxMin.y; P.y <= bboxMax.y; P.y++) {
			if (barycentric(pts, P) == false) { 
				continue; } //breaks current iteration of loop if point is outside triangle
			image.set(P.x, P.y, color);
		}
	}
}
